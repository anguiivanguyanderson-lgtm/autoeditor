...
// Draw an image contained (letterboxed) in WxH.
function paint(ctx, img, W, H) {
...
// as progress p goes 0 -> 1.
export const TRANSITIONS = {
  cut: {
    label: "Cut", icon: "│", xfade: null,
    canvas: (c, f, t, p, W, H) => paint(c, t, W, H),
  },
  fade: {
  ...
  fadeblack: {
    ...
    canvas: (c, f, t, p, W, H) => {
      if (p < 0.5) { c.globalAlpha = 1 - 2 * p; paint(c, f, W, H); }
      else { c.globalAlpha = 2 * p - 1; paint(c, t, W, H); }
      c.globalAlpha = 1;
    },
  },
  wipeleft: {
  ...
  slideleft: {
    ...
    canvas: (c, f, t, p, W, H) => {
      c.save(); c.translate(-W * p, 0); paint(c, f, W, H); c.restore();
      c.save(); c.translate(W * (1 - p), 0); paint(c, t, W, H); c.restore();
    },
  },
  slideright: {
