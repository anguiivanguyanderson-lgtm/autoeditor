...
export function wrapToWidth(text, maxChars) {
  ...
  const n = Math.ceil(clean.length / maxChars);      // lines needed to fit width
  const target = Math.ceil(clean.length / n);        // balance them evenly
  const lines = [];
  let cur = "";
  for (const w of words) {
...
export function drawCaption(ctx, text, W, H, styleId, fontPx, lineHeight) {
  if (!text) return;
