...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  if (!writable && isIOS) {
    if (!canOPFS) {
      log("ios: OPFS not available (old Safari) — in-memory buffer (tab may reload on long renders)");
    } else {
      try { opfsOut = await openOpfsOutput(); }
      catch (e) { opfsOut = null; log(`ios: OPFS output failed (${e && e.message ? e.message : e}) — in-memory buffer (tab may reload on long renders)`); }
    }
  }
