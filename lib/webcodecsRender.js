...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  // NOTE: OPFS output (streaming the muxed MP4 to disk) was tried to keep the output off
  // RAM, but with fastStart:false the muxer patches the header via a backward seek, and
  // that positioned write through an OPFS/FS writable produced corrupt, unplayable MP4s
  // (WebKit especially). Assemble in the in-memory ChunkedBuffer instead — proven to make
  // valid files — until a no-seek approach (fragmented MP4) is verified.
  const streaming = !!writable;
  let chunkedBuf = null, target;
  if (streaming) {
  ...
  log(streaming ? "output: streaming to file" : "output: chunked buffer");
  ...
  const makeVideoEncoder = () => {
    ...
    enc.configure({ codec: profile.videoCodec, width: W, height: H, framerate: fps, bitrate, avc: { format: "avc" } });
    return enc;
  };
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    encoder.encode(frame, { keyFrame: f % kf === 0 || f % SEG === 0 }); // segment starts must be keyframes
    frame.close();
