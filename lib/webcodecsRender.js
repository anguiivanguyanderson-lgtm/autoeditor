...
function drawContain(ctx, bmp, W, H, zoom, alpha) {
  if (!bmp) return;
  const iw = bmp.videoWidth || bmp.naturalWidth || bmp.width;
  const ih = bmp.videoHeight || bmp.naturalHeight || bmp.height;
  if (!iw || !ih) return;
...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel) {
  const bail = () => {
  ...
  const { clips, width: W, height: H, fps = 30, transitions = [], transitionDuration = 0.4, motions = [], motionAmount = 0.08, audioFile = null,
    ...
    videosByName = {}, trims = [], speeds = [], volumes = [] } = spec;
  ...
  const wantAudio = (!!voiceBuffer || clipAudio.length > 0) && !!OAC;
  ...
  async function mixAudio() {
    try {
      ...
      const octx = new OAC(mixChannels, length, mixRate);
      if (voiceBuffer) {
        const s = octx.createBufferSource(); s.buffer = voiceBuffer;
        s.connect(octx.destination); s.start(0);
      }
      const decodeCtx = new (window.AudioContext || window.webkitAudioContext)();
      for (const ca of clipAudio) {
        let cbuf = null;
        try { cbuf = await decodeCtx.decodeAudioData(await ca.file.arrayBuffer()); }
        catch (_) { cbuf = null; } // clip may have no audio track
        if (!cbuf) continue;
        const sr = cbuf.sampleRate;
        const nCh = Math.min(2, cbuf.numberOfChannels);
        // Source region for this slot: from the trim in-point, dur*speed long.
        const startSample = Math.max(0, Math.floor(ca.offset * sr));
        const regionLen = Math.min(cbuf.length - startSample, Math.round(ca.dur * ca.speed * sr));
        if (regionLen <= 0) continue;
        const region = [];
        for (let c = 0; c < nCh; c++) region.push(cbuf.getChannelData(c).subarray(startSample, startSample + regionLen));
        // Pitch-preserving fit-speed: pre-stretch to the slot length, then play at 1x.
        const outChans = Math.abs(ca.speed - 1) < 1e-3 ? region : timeStretch(region, ca.speed);
        const outLen = outChans[0].length;
        const abuf = octx.createBuffer(nCh, outLen, sr);
        for (let c = 0; c < nCh; c++) abuf.copyToChannel(outChans[c], c);
        const s = octx.createBufferSource(); s.buffer = abuf; // already at slot length, so 1x
        const g = octx.createGain(); g.gain.value = ca.vol;
        s.connect(g); g.connect(octx.destination);
        // start()/stop() are in OUTPUT time, so the clip's audio plays for its slot.
        try { s.start(ca.start); s.stop(ca.start + ca.dur); } catch (_) {}
      }
  ...
  // A drawable for clip i at output time t: a seeked <video> for video clips,
  // otherwise the (time-independent) ImageBitmap.
  async function getFrame(i, t) {
    ...
    const src = Math.max(0, Math.min(dur ? dur - 1e-3 : 0, trimStart + Math.max(0, t - c.start) * speed));
    await seekTo(entry.v, src);
    return entry.v;
  }
