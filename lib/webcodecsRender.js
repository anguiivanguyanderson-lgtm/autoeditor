...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  const { clips, fps = 30, transitions = [], transitionDuration = 0.4, motions = [], motionAmount = 0.08, audioFile = null,
    ...
    videosByName = {}, trims = [], speeds = [], volumes = [], bitrate = 8_000_000, forceDiskStream = false } = spec;
  ...
  // Normally only iOS streams to disk. `forceDiskStream` (a ?stream=1 debug flag) forces
  // the same fragmented-OPFS path on any OPFS browser — used to verify on Android/desktop
  // that the fragmented fMP4 is valid and plays, which is the one thing an iPhone-less
  // machine can't check about the iOS path.
  let chunkedBuf = null, target, opfsOut = null;
  if (!writable && (isIOS || forceDiskStream)) {
    const tag = isIOS ? "ios" : "test";
  ...
  const streaming = streamToDisk || !!writable;
  ...
  log(streamToDisk ? "output: fragmented mp4 → OPFS (streaming)" : (streaming ? "output: streaming to file" : "output: chunked buffer"));
  ...
  async function pumpAudioTo(upto) {
    ...
    try {
      ...
      log(`audio failed (${e && e.message ? e.message : e}) — continuing video-only`);
      audioDead = true; audioEnc = null; audioErr = null;          // keep the finished video
    }
  }
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    if ((f + 1) % winFrames === 0) {
      ...
      if (fragmented && audioEnc && !audioDead) {
        try { await audioEnc.flush(); }
        catch (e) { if (e && e.cancelled) throw e; log(`audio failed (${e && e.message ? e.message : e}) — continuing video-only`); audioDead = true; audioEnc = null; }
      }
    }
  ...
  if (wantAudio && !audioDead && audioEnc) {
    report(0.96, "Finalizing audio");
    await pumpAudioTo(totalSamples);
    if (!audioDead && audioEnc) {
      try { await audioEnc.flush(); }
      catch (e) { if (e && e.cancelled) throw e; log(`audio failed (${e && e.message ? e.message : e}) — finalizing video-only`); audioDead = true; }
    }
  }
