...
// Stream the muxed MP4 to an OPFS scratch file on disk instead of holding the whole
// output in memory. This is the last big length-scaling buffer — offloading it keeps
// render memory roughly flat regardless of duration. Returns a target-ready writable
// plus a getBlob() (disk-backed File — no full RAM load) or null if OPFS is unavailable
// (caller then falls back to the in-memory ChunkedBuffer, i.e. current behaviour).
async function createOpfsOutput() {
  if (typeof navigator === "undefined" || !(navigator.storage && navigator.storage.getDirectory)) return null;
  try {
    const root = await navigator.storage.getDirectory();
    // Best-effort sweep of leftovers from earlier renders (we can't delete the current
    // one until its download is done, so we clean up on the next render instead).
    try { if (root.entries) for await (const [nm] of root.entries()) { if (typeof nm === "string" && nm.startsWith("aeout-")) { try { await root.removeEntry(nm); } catch (_) {} } } } catch (_) {}
    const name = `aeout-${Date.now()}-${Math.random().toString(36).slice(2)}.mp4`;
    const fh = await root.getFileHandle(name, { create: true });
    const writable = await fh.createWritable();
    return { writable, getBlob: () => fh.getFile() };
  } catch (_) { return null; }
}
...
async function encodeProbe(codec, width, height, framerate, bitrate) {
  ...
  try {
    ...
    enc = new VideoEncoder({ output: () => { got = true; }, error: (e) => { err = e; } });
...
async function videoCodecOk(codec, width, height, framerate, bitrate) {
  try { const s = await VideoEncoder.isConfigSupported({ codec, width, height, framerate, bitrate }); if (s && s.supported) return true; }
...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  if (audioFile) { try { voiceSource = await createVoiceSource(audioFile); } catch (_) { voiceSource = null; } }
  ...
  // Stream to a file on disk when a writable is provided; otherwise mux into a
  // chunked buffer (no single-ArrayBuffer size ceiling), assembled into a Blob.
  const streaming = !!writable;
  let chunkedBuf = null, target, opfsOut = null;
  if (streaming) {
    target = new FileSystemWritableFileStreamTarget(writable);
  } else {
    opfsOut = await createOpfsOutput(); // disk-backed, keeps the output off RAM
    if (opfsOut) {
  ...
  const muxer = new Muxer({
    ...
    ...(wantAudio ? { audio: { codec: "aac", sampleRate: mixRate, numberOfChannels: mixChannels } } : {}),
    fastStart: false,
  });
  log(streaming ? "output: streaming to file" : opfsOut ? "output: OPFS disk file" : "output: chunked buffer");
  ...
  encoder.configure({ codec: profile.videoCodec, width: W, height: H, framerate: fps, bitrate, avc: { format: "avc" } });
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    while (encoder.encodeQueueSize > 4) { if (encErr) throw encErr; bail(); await yielder.tick(); }
  ...
  // Mix (voiceover + clip audio) and encode the track in short windows — never a full
  // mix buffer — so memory stays flat even for very long timelines, and mux it in.
  if (wantAudio) {
    report(0.9, "Finalizing audio");
    let clipMixes = [];
    try { clipMixes = await prepareClipMixes(); } catch (_) { clipMixes = []; }
    const sampleRate = mixRate;
    const channels = mixChannels;
    if (!voiceSource && !clipMixes.length) {
      log("no audio to encode (video-only track)");
    } else {
      // Audio failing must NOT throw away an already-finished video — log and finalize
      // video-only instead (a user cancel still propagates).
      try {
      let aChunks = 0;
      ...
      const totalSamples = Math.max(1, Math.ceil(total * sampleRate));
      ...
      for (let off = 0; off < totalSamples; off += STEP) {
        ...
        // Voiceover from t=0 — read this window's slice straight from the source (mono
        // duplicated to both output channels).
        if (voiceSource) {
          const vwin = await voiceSource.read(off, n);
          for (let c = 0; c < channels; c++) planar.set(vwin[Math.min(c, vwin.length - 1)], c * n);
        }
        // Add each clip's audio where it overlaps this window.
        for (const cm of clipMixes) {
          const cs = cm.at, ce = cm.at + cm.chans[0].length;
          if (ce <= off || cs >= off + n) continue; // no overlap with [off, off+n)
          const lo = Math.max(off, cs), hi = Math.min(off + n, ce);
          for (let c = 0; c < channels; c++) {
            const s = cm.chans[Math.min(c, cm.chans.length - 1)];
            const base = c * n;
            for (let gi = lo; gi < hi; gi++) planar[base + (gi - off)] += s[gi - cs] * cm.vol;
          }
        }
        ...
        while (audioEnc.encodeQueueSize > 8) { if (encErr) throw encErr; bail(); await yielder.tick(); }
      }
      await audioEnc.flush();
      if (encErr) throw encErr;
      log(`audio encoded: ${aChunks} chunk(s)${aChunks === 0 ? " — WARNING: encoder produced no audio" : ""}`);
      } catch (e) {
        if (e && e.cancelled) throw e; // user cancel → propagate
        log(`audio failed (${e && e.message ? e.message : e}) — finalizing video-only`);
        encErr = null; // keep the finished video; drop the audio error
      }
    }
  }
  ...
  if (opfsOut) {
    await opfsOut.writable.close();          // flush the muxed file to disk
    const blob = await opfsOut.getBlob();    // disk-backed File — not loaded into RAM
    log(`done: ${(blob.size / (1024 * 1024)).toFixed(1)} MB (OPFS)`);
    report(1, "Done");
    return blob; // caller downloads it; the scratch file is swept on the next render
  }
