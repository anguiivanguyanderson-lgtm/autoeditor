...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  log(`video ${W}x${H} @ ${fps}fps, ${total.toFixed(1)}s, ${totalFrames} frames, codec ${profile.videoCodec}, bitrate ${(effBitrate / 1e6).toFixed(1)} Mbps${effBitrate < bitrate ? " (capped for in-memory output)" : ""}`);
  ...
  const makeVideoEncoder = () => {
    ...
    enc.configure({ codec: profile.videoCodec, width: W, height: H, framerate: fps, bitrate: effBitrate, avc: { format: "avc" } });
    return enc;
  };
