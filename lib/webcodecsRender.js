...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  if (wantAudio) {
    ...
    // Voiceover channels, read window-by-window (no copy of the whole track).
    const voiceChans = voiceBuffer
      ? Array.from({ length: channels }, (_, c) => voiceBuffer.getChannelData(Math.min(c, voiceBuffer.numberOfChannels - 1)))
      : null;
    if (!voiceChans && !clipMixes.length) {
      ...
      for (let off = 0; off < totalSamples; off += STEP) {
        ...
        if (voiceChans) {
          for (let c = 0; c < channels; c++) {
            const src = voiceChans[c];
            const lim = Math.min(n, src.length - off);
            if (lim > 0) planar.set(src.subarray(off, off + lim), c * n);
          }
        }
