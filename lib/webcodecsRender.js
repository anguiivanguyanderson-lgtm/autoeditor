...
// The API existing isn't enough — Firefox exposes VideoEncoder but has NO H.264
// encoder, so it "supports" the API yet fails at encode. Actually probe H.264
// encode support so the Fast render is only offered where it truly works.
export async function webCodecsCanEncodeH264() {
  if (!webCodecsSupported() || typeof VideoEncoder.isConfigSupported !== "function") return false;
  const codecs = ["avc1.42001f", "avc1.4d0028", "avc1.640028"];
  for (const codec of codecs) {
    try {
      const s = await VideoEncoder.isConfigSupported({ codec, width: 640, height: 480, framerate: 30, bitrate: 1_000_000 });
      if (s && s.supported) return true;
    } catch (_) { /* try next */ }
  }
  return false;
}
...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  // Cap to 1080p: AVC encoder levels and browser memory don't handle arbitrary
  // huge source resolutions (e.g. 2752x1536), and >1080p isn't needed for delivery.
  let W = spec.width, H = spec.height;
  { const s = Math.min(1, 1920 / W, 1080 / H); if (s < 1) { W = Math.max(2, Math.round((W * s) / 2) * 2); H = Math.max(2, Math.round((H * s) / 2) * 2); } }
  ...
  const mixChannels = 2;
  // Only declare an audio track if this browser can actually AAC-encode (Safari's
  // WebCodecs AAC support is unreliable) — otherwise a declared-but-empty track
  // makes the file broken/silent. A clean video-only file is the safe fallback.
  let aacOk = typeof AudioEncoder !== "undefined";
  if (aacOk) {
    try {
      const sup = await AudioEncoder.isConfigSupported({ codec: "mp4a.40.2", sampleRate: mixRate, numberOfChannels: mixChannels, bitrate: 192000 });
      aacOk = !!(sup && sup.supported);
    } catch (_) { aacOk = false; }
  }
  const wantAudio = (!!voiceBuffer || clipAudio.length > 0) && !!OAC && aacOk;
  log(`aac-encode supported: ${aacOk}`);
  ...
  // size ceiling — needed for long videos whose MP4 exceeds the max ArrayBuffer).
  const streaming = !!writable;
  const muxer = new Muxer({
    target: streaming ? new FileSystemWritableFileStreamTarget(writable) : new ArrayBufferTarget(),
  ...
  log(`video ${W}x${H} @ ${fps}fps, ${total.toFixed(1)}s, ${totalFrames} frames, codec ${codec}, bitrate ${(bitrate / 1e6).toFixed(1)} Mbps`);
  ...
  encoder.configure({ codec, width: W, height: H, framerate: fps, bitrate, avc: { format: "avc" } });
