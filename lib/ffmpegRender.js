...
async function getFFmpeg() {
  ...
  // Drive progress from the "time=" in ffmpeg's status log against the known
  // total — the core's own progress event is unreliable on the MT build.
  ffmpeg.on("log", ({ message }) => {
    if (!_onProgress || !_total) return;
    const m = message.match(/time=\s*(\d+):(\d+):([\d.]+)/);
    if (m) {
