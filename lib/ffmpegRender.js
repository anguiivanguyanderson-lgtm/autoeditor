...
import { drawtextChain, sanitizeCueText, captionCueFile, CAPTION_FONT } from "./captions";
...
// Write the font + one textfile per cue and return the drawtext chain, or "".
async function setupCaptions(ffmpeg, captions, captionStyle, captionSize, width, height) {
  ...
  for (let i = 0; i < captions.length; i++) {
    await ffmpeg.writeFile(captionCueFile(i), new TextEncoder().encode(sanitizeCueText(captions[i].text)));
  }
  return drawtextChain(captions, captionStyle, width, height, captionSize);
}
