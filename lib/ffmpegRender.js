...
async function renderConcat(ffmpeg, { clips, paths, audioFs, width, height, fps }) {
  let concat = "";
  for (let i = 0; i < clips.length; i++) {
...
async function renderXfade(ffmpeg, { clips, paths, audioFs, width, height, fps, transitions, transitionDuration }) {
  const n = clips.length;
  const frame = 1 / fps;
  const clamp = (d) => Math.min(MAX_TRANSITION_DURATION, Math.max(MIN_TRANSITION_DURATION, d));

  // Effective transition for the incoming edge of clip k. A "cut" becomes a
  // single-frame fade so the whole timeline is one uniform xfade chain.
  const tdur = (k) => (!transitions[k] || transitions[k] === "cut" ? frame : clamp(transitionDuration));
  const tname = (k) => transitionOf(transitions[k]).xfade || "fade";

  const total = clips[n - 1].start + clips[n - 1].duration;

  const inputs = [];
  const parts = [];
  for (let i = 0; i < n; i++) {
    // Clip i's stream must live from its own start until the next transition ends.
  ...
  for (let k = 1; k < n; k++) {
    ...
    parts.push(
      `[${prev}][v${k}]xfade=transition=${tname(k)}:duration=${tdur(k).toFixed(3)}:offset=${clips[k].start.toFixed(3)}[${out}]`
    );
    prev = out;
  }
...
export async function renderVideo(opts) {
  const {
    ...
    transitions, transitionDuration = 0.4, onProgress,
  } = opts;
  ...
  const useXfade = Array.isArray(transitions) && clips.length >= 2 &&
    transitions.some((t, i) => i > 0 && t && t !== "cut");

  const args = { clips, paths, audioFs, width, height, fps };
  return useXfade
    ? renderXfade(ffmpeg, { ...args, transitions, transitionDuration })
