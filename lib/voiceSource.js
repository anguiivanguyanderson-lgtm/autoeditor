...
export async function createVoiceSource(file) {
  if (!file) return null;
  try { const w = await wavSource(file); if (w && w.length > 0) return w; } catch (_) {}
  try { return await decodedSource(file); } catch (_) { return null; }
}
